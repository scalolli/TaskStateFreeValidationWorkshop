...
object CatsState {
  ...
  /**
    ...
    */
  def getExchangeRate(from: Currency, to: Currency): State[CurrencyExchangeRates, Option[Double]] = State {
    state: CurrencyExchangeRates =>
      val calculatedValue = state.getRate(from, to)
  ...
  def getDollarExchangeRate(to: Currency): State[CurrencyExchangeRates, Option[Double]] =
    State[CurrencyExchangeRates, Option[Double]] {
      state => (state, state.getRate(Currency.USD, to))
    }
  ...
  /**
    ...
    */
  def setDollarExchangeRate(to: Currency, rate: Double): State[CurrencyExchangeRates, Option[Double]] = State[CurrencyExchangeRates, Option[Double]] {
    state =>
      val oldRate = state.getRate(Currency.USD, to)
      val newState = state.dollarExchangeRates.updated(to, rate)
      (CurrencyExchangeRates(newState), oldRate)
